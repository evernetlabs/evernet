...
class EventService:
    ...
    def create(self, node_identifier: str, structure_address: str, identifier: str, display_name: str, description: str, json_schema: str, creator: str):
        ...
        if not is_valid_json_schema_definition(json_schema):
            raise ClientException(f"Invalid JSON schema provided for event {identifier} of structure {structure_address} on node {node_identifier}")
        ...
        if self.mongo.count_documents({
            ...
            raise ClientException(f"Event {identifier} already exists for structure {structure_address} on node {node_identifier}")
    ...
    def fetch(self, node_identifier: str, structure_address: str) -> list[dict]:
        events = self.mongo.find({"node_identifier": node_identifier, "structure_address": structure_address})
        return [self.to_dict(event) for event in events]

    def get(self, node_identifier: str, structure_address, identifier: str) -> dict:
        event = self.mongo.find_one({
            "node_identifier": node_identifier,
            "structure_address": structure_address,
            "identifier": identifier
        })
        ...
        if not event:
            raise NotFoundException(f"Event {identifier} not found for structure {structure_address} on node {node_identifier}")
